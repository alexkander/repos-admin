...
import { BranchConstants } from '../branch/branch.constants';
import { GitBranchType, GitReferenceType, GitRemoteType, GitTagType } from './git-repo.types';
...
export class GitRepo {
  ...
  async getBranches(): Promise<GitBranchType[]> {
    ...
    const branches = Object.values(branchesInfo.branches).map((branch) => {
      ...
      const shortName = remoteName ? largeName.substring(remotePrefix.length) : largeName;
  ...
  async isReferenceBackedUp(ref: GitReferenceType, references: GitReferenceType[]) {
  ...
  async isLocalReferenceBackedUp(remoteRef: GitReferenceType, references: GitReferenceType[]) {
  ...
  async isRemoteReferenceBackedUp(ref: GitReferenceType, references: GitReferenceType[]) {
    const remoteReferences = references.filter(
      (rr) => rr.remoteName && rr.remoteName !== ref.remoteName && rr.shortName === ref.shortName,
    );
    for (const remoteRef of remoteReferences) {
      const isDescendent = await this.isDescendent(remoteRef?.commit, ref.commit);
      if (isDescendent) {
        return true;
      }
    }
  ...
  async isDescendent(childCommit: string, parentCommit: string) {
    ...
    const ancestor = await this.handler.raw([`merge-base`, parentCommit, childCommit]);
  ...
  async getTags(pRemoteNames?: string[]): Promise<GitTagType[]> {
    ...
    const remotesTagsPromises = remoteNames.map((remoteName) => this.listRemoteTags(remoteName));
